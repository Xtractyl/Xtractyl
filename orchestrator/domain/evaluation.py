...
def evaluate_projects(cmd: EvaluateProjectsCommand, project_repo=None, run_repo=None) -> dict:
    if USE_DB_BACKEND and project_repo and run_repo:
        return _evaluate_projects_db(cmd, project_repo, run_repo)
    return _evaluate_projects_legacy(cmd)


def _evaluate_projects_db(cmd: EvaluateProjectsCommand, project_repo, run_repo) -> dict:
...
def _evaluate_projects_legacy(cmd: EvaluateProjectsCommand) -> dict:
    """
...
def get_groundtruth_qals(project_repo=None) -> dict:
    if USE_DB_BACKEND and project_repo:
        return _get_groundtruth_qals_db(project_repo)
    return _get_groundtruth_qals_legacy()


def _get_groundtruth_qals_db(project_repo) -> dict:
...
def _get_groundtruth_qals_legacy() -> dict:
    """
...
def save_as_gt_set(cmd: SaveAsGtSetCommand, project_repo=None) -> dict:
    if USE_DB_BACKEND and project_repo:
        return _save_as_gt_set_db(cmd, project_repo)
    return _save_as_gt_set_legacy(cmd)


def _save_as_gt_set_db(cmd: SaveAsGtSetCommand, project_repo) -> dict:
