...
def export_final_annotations_main():
    logs = []
    ...
    logs.append(f"📥 Lade finale Annotationen aus Projekt {PROJECT_ID}")
    tasks = fetch_tasks(PROJECT_ID)
    logs.append(f"✅ {len(tasks)} Tasks mit Annotationen gefunden")
    ...
    logs.append(f"✅ Finale Annotationen gespeichert unter: {SAVE_PATH}")
    return logs


def export_final_annotations_main_wrapper():
    return export_final_annotations_main()

if __name__ == "__main__":
    for line in export_final_annotations_main():
        print(line)